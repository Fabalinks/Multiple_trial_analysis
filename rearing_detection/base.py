...
def read_data(root_path, tag, has_beacon=True, has_metadata=True):
    ...
    for p in meta_paths:
        if tag in p:
            metadata = pd.read_csv(p, sep=" : ", header=None, index_col=0, engine='python').T
...
def make_trials(position_data, beacon_data, metadata):
    ...
    sampling_rate = position_data[1, 0] - position_data[0, 0] # it coule be 1/(time interval)
...
class BeaconPosition():
    ...
    def __init__(self, root_path, tag, has_beacon=True, has_metadata=True):
        ...
        self.position_data=rotation_correction(self.position_data.to_numpy()[:, [0,1,3,2,4,5,6]]) # add another 3 column Miao 
